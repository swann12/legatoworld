  ...
  return (
    <Shell livingBg={false}>
      <div className="min-h-dvh bg-paper text-dusk pb-32">
        ...
        <section className="px-5 pt-6"><IvoryCard><textarea value={body} onChange={(e) => setBody(e.target.value)} rows={8} placeholder="Une lettre, une pensée, sans destinataire…" className="w-full bg-transparent resize-none outline-none px-5 py-5 font-serif italic text-[17px] leading-[28px] text-dusk placeholder:text-dusk/30" /></IvoryCard><button onClick={save} disabled={!body.trim()} className="mt-4 w-full rounded-full px-6 py-4 disabled:opacity-40" style={{ background: "var(--terracotta)", color: "var(--paper)" }}>Garder →</button></section>
