  ...
  return (
    <Shell livingBg={false}>
      <div className="min-h-dvh bg-paper text-dusk pb-32">
        <header className="px-6 pt-7 flex items-center justify-between"><LegatoMark to="/space" size={22} /><Link to="/care/journal" className="mono-label text-dusk/55">Lettre →</Link></header>
        ...
        <section className="px-5 pt-8 space-y-3">{dates.map((d) => <div key={d.id} className="rounded-[18px] border border-dusk/12 bg-[color:var(--whisper)] px-5 py-4"><p className="font-serif text-[18px]">{d.label}</p><p className="mt-1 text-[12.5px] text-dusk/60">{d.daysAway === 0 ? "Aujourd'hui" : `Dans ${d.daysAway} jours`} · lettre, voix, rituel ou contact proche</p></div>)}<button className="w-full rounded-[18px] border border-dusk/12 px-5 py-4 font-serif italic">Ajouter une date</button></section>
      </div>
    </Shell>
  );
}