...
export const Route = createFileRoute("/care/memory")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Le jardin, les voix, les lettres, les dates qui comptent — dans votre espace de soutien." },
    ],
  }),
  component: CareMemory,
});

function CareMemory() {
  const { situation, primaryNeed, stage, lovedOneRelation, legallyInvolved, hydrated } = useLegato();
  const lovedName = useLovedName();
  const { memory } = journeyModules(situation, primaryNeed, stage, { relation: lovedOneRelation, legallyInvolved });
  const dates = hydrated ? upcomingSensitiveDates({ windowDays: 7 }) : [];

  return (
    <Shell livingBg={false}>
      <div className="min-h-dvh bg-paper text-dusk pb-32">
        <header className="px-6 pt-7 flex items-center justify-between">
          <Link to="/care" className="mono-label">← Soutien</Link>
        ...
        <section className="px-6 pt-10">
          ...
          <p className="mt-5 text-[13.5px] leading-[1.6] text-dusk/60 max-w-[34ch]">
            Voix, photos, mots, dates — ici, rien ne s'efface.
          </p>
        </section>
        ...
        {dates.length > 0 && (
          <section className="px-5 pt-8">
            ...
            <div className="mt-3 rounded-[18px] border border-dusk/12 bg-[color:var(--whisper)] px-5 py-4">
              ...
              <Link to="/care/dates" className="mt-3 inline-block mono-label" style={{ color: "var(--terracotta)" }}>
                Préparer un geste →
              </Link>
            </div>
          </section>
        ...
        <section className="px-5 pt-8 flex flex-col gap-3">
          {(memory.length ? memory : (["garden", "voices", "letters", "dates", "timeline"] as MemoryModule[])).map((m) => {
            const cfg = MEMORY_LABELS[m];
            return (
              <Link
                key={m}
                to={cfg.to as "/care/garden"}
